...
TRUST_SOURCE='OUT_OF_BAND_PREPROD_CONSOLE'
PINNED_KEY_PATH='scripts/preproduction-ssh-trust/preprod-ed25519.pub'
PINNED_FINGERPRINT_PATH='scripts/preproduction-ssh-trust/preprod-ed25519.sha256'

if [[ ! -f "$PINNED_KEY_PATH" || ! -f "$PINNED_FINGERPRINT_PATH" ]]; then
  echo 'Pinned PREPROD SSH trust material is incomplete.' >&2
  exit 65
fi
...
if [[ "$PINNED_KEY_TYPE" != 'ssh-ed25519' ]] || [[ -z "$PINNED_KEY_BLOB" ]] || [[ -n "${PINNED_KEY_EXTRA:-}" ]]; then
  echo 'Pinned PREPROD SSH public key has an unexpected format.' >&2
  exit 66
fi

EXPECTED_FINGERPRINT="$(tr -d '\r\n' < "$PINNED_FINGERPRINT_PATH")"
if [[ ! "$EXPECTED_FINGERPRINT" =~ ^SHA256:[A-Za-z0-9+/]+$ ]]; then
  echo 'Pinned PREPROD SSH fingerprint has an unexpected format.' >&2
  exit 67
fi

PINNED_FINGERPRINT="$(ssh-keygen -lf "$PINNED_KEY_PATH" -E sha256 | awk '{print $2}')"
if [[ "$PINNED_FINGERPRINT" != "$EXPECTED_FINGERPRINT" ]]; then
  echo 'Pinned PREPROD SSH key does not match the pinned fingerprint.' >&2
  exit 68
fi

PREPROD_SERVER_HOST="${PREPROD_SERVER_HOST:-}"
if [[ ! "$PREPROD_SERVER_HOST" =~ ^[A-Za-z0-9._:-]+$ ]]; then
  echo 'PREPROD_SERVER_HOST is invalid.' >&2
  exit 69
fi
...
matching_entries() {
  ...
  ssh-keygen -F "$PREPROD_SERVER_HOST" -f "$KNOWN_HOSTS" 2>/dev/null | grep -v '^#' || true
}
...
validate_single_entry() {
  ...
  if [[ "$count" -ne 1 ]]; then
    echo 'PREPROD_SERVER_HOST must resolve to exactly one known_hosts entry.' >&2
    exit 72
  fi
  ...
  if [[ -z "$host_field" || "$key_type" != 'ssh-ed25519' || -z "$key_blob" || -n "${extra:-}" ]]; then
    echo 'PREPROD_SERVER_HOST known_hosts entry has an unexpected format or key type.' >&2
    exit 73
  fi
  if [[ "$key_blob" != "$PINNED_KEY_BLOB" ]]; then
    echo 'PREPROD_SERVER_HOST known_hosts key does not match the pinned PREPROD key.' >&2
    exit 74
  fi
  ...
  if [[ "$actual_fingerprint" != "$EXPECTED_FINGERPRINT" ]]; then
    echo 'PREPROD_SERVER_HOST known_hosts fingerprint does not match the pinned PREPROD fingerprint.' >&2
    exit 75
  fi

  printf 'ENTRY_COUNT_FOR_PREPROD_SERVER_HOST=1\n'
...
if [[ "$count_before" -gt 1 ]]; then
  echo 'PREPROD_SERVER_HOST has multiple known_hosts entries; refusing ambiguity.' >&2
  exit 76
fi
...
if [[ "$count_before" -eq 0 ]]; then
  if [[ "$MODE" == 'VERIFY_ONLY' ]]; then
    echo 'Pinned PREPROD host identity is absent from known_hosts.' >&2
    exit 77
  fi
  ...
  printf '%s %s %s\n' "$PREPROD_SERVER_HOST" "$PINNED_KEY_TYPE" "$PINNED_KEY_BLOB" >> "$KNOWN_HOSTS"
else
