...
if [[ "$#" -ne 4 ]] || [[ "$ACTION" != 'PRECHECK' && "$ACTION" != 'IMPORT' && "$ACTION" != 'IMPORT_SANITIZE_PROVE' && "$ACTION" != 'CLEANUP' && "$ACTION" != 'VERIFY_ABSENCE' ]]; then
...
[[ "$helper_identity" == '0:0:755' ]] || { echo 'Privileged helper owner/group/mode does not match repository contract.' >&2; exit 70; }
actual_helper_sha256="$(sha256sum "$PRIVILEGED_HELPER" | awk '{print $1}')"
[[ "$actual_helper_sha256" == "$EXPECTED_HELPER_SHA256" ]] || { echo 'Privileged helper digest does not match repository authority.' >&2; exit 71; }
...
runtime_isolated() {
  test -L "$CURRENT_LINK" && test -L "$ACTIVE_SETTINGS" && \
...
capacity_check() {
  ...
  multiplier=4; margin=1073741824; minimum=2147483648
  required=$((SNAPSHOT_BYTES * multiplier + margin))
  if [[ "$required" -lt "$minimum" ]]; then required="$minimum"; fi
...
case "$ACTION" in
  PRECHECK)
    ...
    printf '%s\n' 'preprod_capacity=PASS' 'preprod_runtime_points_to_staging=NO' 'staging_admin_capability=PASS' 'privileged_helper_identity=PASS' 'privileged_helper_digest=PASS'
    ;;
  VERIFY_ABSENCE)
    ...
    grep -Fxq 'staging_account_present_after_cleanup=NO' <<< "$helper_out"
    printf '%s\n' 'preprod_runtime_points_to_staging=NO' 'staging_db_present_after_cleanup=NO' 'staging_account_present_after_cleanup=NO' 'privileged_helper_identity=PASS' 'privileged_helper_digest=PASS'
    ;;
  CLEANUP)
    helper_out="$(run_helper)"
    grep -Fxq 'staging_db_present_after_cleanup=NO' <<< "$helper_out"
    ...
    printf '%s\n' 'preprod_runtime_points_to_staging=NO' 'staging_db_present_after_cleanup=NO' 'staging_account_present_after_cleanup=NO' 'privileged_helper_identity=PASS' 'privileged_helper_digest=PASS'
    ;;
  IMPORT|IMPORT_SANITIZE_PROVE)
    [[ "$SNAPSHOT_BYTES" -gt 0 ]] || { echo 'Import action requires a positive snapshot byte count.' >&2; exit 76; }
    ...
    if [[ "$ACTION" == 'IMPORT' ]]; then
      ...
      grep -Fxq 'one_shot_import_sanitize_prove_cleanup=PASS' <<< "$helper_out"
      grep -Fxq 'sanitization_policy=agency-preprod-refresh-v1' <<< "$helper_out"
    fi
    grep -Fxq 'exact_byte_import=PASS' <<< "$helper_out"
    grep -Eq '^safe_table_count=[1-9][0-9]*$' <<< "$helper_out"
    grep -Fxq 'staging_db_present_after_cleanup=NO' <<< "$helper_out"
    grep -Fxq 'staging_account_present_after_cleanup=NO' <<< "$helper_out"
    runtime_isolated || { echo 'PREPROD runtime points to staging after bounded import action.' >&2; exit 79; }
    printf '%s\n' 'preprod_runtime_points_to_staging_before=NO'
    printf '%s\n' "$helper_out"
    printf '%s\n' 'preprod_runtime_points_to_staging_after=NO' 'privileged_helper_identity=PASS' 'privileged_helper_digest=PASS'
    ;;
esac
