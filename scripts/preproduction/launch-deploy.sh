...
LOCK_FILE="/var/www/agency-preprod/shared/deploy.lock"
REQUEST_DIR="$JOBS_DIR/$REQUEST_ID"
DEPLOY_SCRIPT="$REQUEST_DIR/deploy-candidate.sh"
...
set +e
"$DEPLOY_SCRIPT" "$EXPECTED_SHA" "$EXPECTED_ARTIFACT_SHA256" "$REQUEST_DIR" >> "$OUTPUT_FILE" 2>&1
status="$?"
set -e
if [[ "$status" -eq 0 ]]; then
