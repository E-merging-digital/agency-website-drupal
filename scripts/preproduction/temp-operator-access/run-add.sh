...
PROFILE_ID='agency-preprod-temp-key-add-v1'
BASE='scripts/preproduction/temp-operator-access'
PROFILE="$BASE/profile.json"
REMOTE="$BASE/remote-add-key.py"
...
[[ "$PREPROD_SSH_KEY" == "$RUNNER_TEMP/agency-899-key-${GITHUB_RUN_ID}."*".key" ]]
trust_home="${PREPROD_KNOWN_HOSTS_FILE%/.ssh/known_hosts}"
[[ "$trust_home" == "$RUNNER_TEMP/agency-899-home-${GITHUB_RUN_ID}."* ]]
...
jq -e '
 .issue_number == 899 and
 .profile_id == "agency-preprod-temp-key-add-v1" and
...
remote_dir="/home/agency-preprod/.agency-899-${suffix}"
remote_script="$remote_dir/remote-add-key.py"
raw="$RUNNER_TEMP/agency-899-${GITHUB_RUN_ID}.raw"
err="$RUNNER_TEMP/agency-899-${GITHUB_RUN_ID}.stderr"
...
if lines[2] not in {"KEY_PRESENT_BEFORE=YES", "KEY_PRESENT_BEFORE=NO"}:
    raise SystemExit(1)
if lines[3] != "KEY_PRESENT_AFTER=YES" or lines[4] != "RESULT=PASS":
    raise SystemExit(1)
PY
