#!/usr/bin/env python3
"""Fail-closed successor authority validator for #912 temporary operator-key REMOVE."""
...
COMMAND = "/agency-preprod-temp-key-remove"
MARKER_RE = re.compile(
    r"(?m)^AGENCY_PREPROD_TEMP_KEY_REMOVE_AUTHORITY\n"
    r"parent_issue=(\d+)\n"
    r"implementation_issue=(\d+)\n"
    r"allowed_action=remove$"
)
...
def validate(args: Any) -> dict[str, str]:
    ...
    require(args.authority_issue_number != IMPLEMENTATION_ISSUE, "#912 cannot authorize execution")
    ...
    prefix = f"issue-{args.authority_issue_number}-remove-{args.live_main[:8]}-r"
...
def main() -> int:
    ...
    print("SUCCESSOR_TEMP_KEY_REMOVE_AUTHORITY=PASS")
    print("#912_LIVE_EXECUTION_AUTHORITY=FORBIDDEN")
    return 0
