...
' "$rc" "$stderr_file" "$@" 2>/dev/null)" || result='UNKNOWN INTERNAL_PARSER_ERROR'
  ...
  case "$result" in
    ...
    'UNKNOWN EXIT_ONE_POLICY_EMPTY'|'UNKNOWN EXIT_ONE_POLICY_EXACT'|\
    'UNKNOWN EXIT_ONE_POLICY_OTHER'|'UNKNOWN EXIT_OTHER_POLICY_EMPTY'|\
    'UNKNOWN EXIT_OTHER_POLICY_EXACT'|'UNKNOWN EXIT_OTHER_POLICY_OTHER'|\
    'UNKNOWN POLICY_EMPTY'|\
...
valid_reasons = {
    ...
    'STDERR_PASSWORD_REQUIRED', 'STDERR_PRESENT',
