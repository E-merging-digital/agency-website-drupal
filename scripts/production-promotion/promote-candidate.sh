...
log() {
  printf '[prod-promote] %s\n' "$1"
}

fail() {
  printf '[prod-promote] ERROR: %s\n' "$1" >&2
  exit 1
}
...
if (( ${#releases[@]} > 3 )); then
  for release in "${releases[@]:3}"; do
    ...
    if [[ "$(readlink -f "$CURRENT_LINK")" != "$(readlink -f "$release_path")" && "$(readlink -f "$ACTIVE_RELEASE")" != "$(readlink -f "$release_path")" ]]; then
      rm -rf "$release_path" || log "Warning: stale release retention cleanup failed for $release_path."
    fi
  done
fi
...
if (( ${#backups[@]} > 10 )); then
  for backup in "${backups[@]:10}"; do
    [[ "$BACKUPS_DIR/$backup" == "$DB_BACKUP" ]] || rm -f "$BACKUPS_DIR/$backup" || true
  done
fi
